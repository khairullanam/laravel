<x-app-layout>
    ...
    <!-- Tombol Tambah -->
    <div class="py-8 px-8">
       <a href="/buku/create"> <button class="px-4 py-2 bg-black text-white rounded-md">Tambah</button></a>
    </div>
            ...
            <tbody>
                @foreach ($ebooks as $post)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-4 py-4">{{ $post['file'] }}</td>
                        <td class="px-4 py-4 content">{{ $post['size'] }}</td>
                        <td class="px-4 py-4">{{ $post['kategori'] }}</td>
                        <td class="px-4 py-4">{{ date('Y-m-d', strtotime($post['published_at'])) }}</td>
                        <td class="px-4 py-4 text-right flex justify-end gap-2">
                            <!-- Tombol Edit -->
                            <a href="{{ route('buku.edit', $post->id) }}" 
                            ...
                            <div id="popup-modal-{{ $post->id }}" tabindex="-1" 
                                ...
                                <div class="relative w-full max-w-md bg-white rounded-lg shadow-md dark:bg-gray-800">
                                    ...
                                    <div class="p-6 text-center">
                                        ...
                                        <form action="{{ route('ebook.destroy', $post->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" 
    ...
    <script>
        ...
        document.querySelectorAll('.content').forEach(function(content) {
            ...
            if (words.length > 10) {
                content.innerText = words.slice(0, 10).join(' ') + '...';
            }
        });
    </script>
